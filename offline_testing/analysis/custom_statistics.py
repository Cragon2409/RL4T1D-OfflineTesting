...
def get_summary_stats(experiment_name, cohort, algo_type, algorithm, algoAbbreviation, metric=['mean', 'std','min', 'max'],
    ...
    for sub in subjects:
        ...
        for s in seeds:
            ...
            for i in range(0, n_trials):
                test_i = 'worker_episode_'+str(5000+i)+'.csv'
                df = pd.read_csv(MAIN_PATH +FOLDER_PATH+ '/'+test_i, names=COL_NAMES)

                print("CGM:", [i for i in df["cgm"]])
                normo, hypo, sev_hypo, hyper, lgbi, hgbi, ri, sev_hyper = time_in_range([float(i) for i in df['cgm'][1:]])
